...
GROUP BY
  PRODUCTLINE
ORDER BY
  Revenue DESC; -- Clasic cars are giving us the ,most revenue
...
GROUP BY
  YEAR_ID
ORDER BY
  Revenue DESC; -- 2004 has the highest revenue
...
GROUP BY
  DEALSIZE
ORDER BY
  Revenue DESC;-- Medium deals are generating the most revenue
...
-- Let's find what productlind in November driving sales
select  
...
SELECT c.*, rfm_recency + rfm_frequency + rfm_monetory AS rfm_column,
